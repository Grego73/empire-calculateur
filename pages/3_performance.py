...
st.subheader("📋 Importation des Tableaux du Jour")
tab_finance = st.text_area("1. Collez le tableau Finance complet :", height=150, key="perf_fin")
tab_capital = st.text_area("2. Collez le tableau Capital complet :", height=150, key="perf_cap")
tab_frais = st.text_area("3. Collez le tableau Frais de Gestion (Optionnel) :", height=150, key="perf_fr")

if st.button("🔍 Générer les Statistiques Globales", use_container_width=True):
    if not tab_finance.strip() or not tab_capital.strip():
        st.error("❌ Les tableaux Finance et Capital sont requis pour générer les statistiques.")
    else:
        try:
